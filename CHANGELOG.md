...
- **Benchmark gate (stage 8):** `scripts/benchmark.sh` now runs ten tracked benchmarks against the frozen `tests/testdata/benchmark/medium.jsonl` (never the live tracker), writes `benchmarks/baseline.txt` with a provenance header (date, Go, CPU, OS, commit, dataset hash), and compares median `ns/op` per benchmark with a built-in comparator (benchstat optional); `tests/scripts/benchmark_compare_test.sh` proves it turns red on a doubled median and a missing benchmark.
