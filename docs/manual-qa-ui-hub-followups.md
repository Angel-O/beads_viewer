...
| Resolved | Current repository scope picker | Open Repository Scope, inspect the current repository styling and controls, then press `c` | The launch repository name is accented and underlined; `c` selects only it as draft state; full controls fit at normal width while narrow layouts remain bounded | Users had to infer the current repository from IDs or paths, had no current-only shortcut, and the controls line was truncated | Manually verified in PR #48 |
