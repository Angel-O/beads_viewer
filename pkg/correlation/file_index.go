...
func BuildCoChangeMatrix(report *HistoryReport) *CoChangeMatrix {
	...
	for _, history := range report.Histories {
		for _, commit := range history.Commits {
			...
			processedCommits[commitIdentity] = true
			...
			// Keep the repository qualifier so identical SHAs from different
			// external repositories remain distinct.
			matrix.CommitFiles[CommitIdentity(commit.Repository, commit.SHA)] = files
