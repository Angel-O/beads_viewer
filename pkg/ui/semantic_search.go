...
func (s *SemanticSearch) computeSemanticResults(term string) ([]list.Rank, map[string]SemanticScore, error) {
	snap := s.Snapshot()
	return s.computeSemanticResultsFromSnapshot(term, snap)
}

func (s *SemanticSearch) computeSemanticResultsFromSnapshot(term string, snap semanticSearchSnapshot) ([]list.Rank, map[string]SemanticScore, error) {
	if !snap.Ready || snap.Index == nil || snap.Embedder == nil {
		return nil, nil, nil
	}
...
func ComputeSemanticFilterCmd(s *SemanticSearch, term string, generations ...uint64) tea.Cmd {
	snapshot := s.Snapshot()
	var dataGen, queryGen uint64
	if len(generations) > 0 {
		dataGen = generations[0]
	}
	if len(generations) > 1 {
		queryGen = generations[1]
	}
	return func() tea.Msg {
		results, scores, err := s.computeSemanticResultsFromSnapshot(term, snapshot)
		return SemanticFilterResultMsg{
			DataGeneration: dataGen, QueryGeneration: queryGen,
			Term: term, Results: results, Scores: scores, Error: err,
...
func BuildSemanticIndexCmd(issues []model.Issue, args ...any) tea.Cmd {
	...
	if len(args) == 4 {
		...
		case int:
			dataGen = uint64(value)
			if generation, ok := args[1].(int); ok {
				buildGen = uint64(generation)
			}
		}
	}
