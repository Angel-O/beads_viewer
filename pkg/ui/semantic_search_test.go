...
func TestModelStartedSemanticIndexUsesRuntimeHubPaths(t *testing.T) {
	root := t.TempDir()
	dataset := writeUISemanticDataset(t, filepath.Join(root, "source"))
	store := filepath.Join(root, "hub", ".beads")
	if err := os.MkdirAll(store, 0o755); err != nil {
		t.Fatal(err)
	}
	...
	command := m.startSemanticIndexBuild()
	if command == nil {
		t.Fatal("model-started semantic build was not scheduled")
	}
	raw := command()
	ready, ok := raw.(SemanticIndexReadyMsg)
	if !ok {
		t.Fatalf("semantic build returned %T, want SemanticIndexReadyMsg", raw)
	}
	if ready.Error != nil {
		t.Fatal(ready.Error)
	}
	wantPrefix := filepath.Join(root, "hub", "semantic") + string(filepath.Separator)
	if !strings.HasPrefix(ready.IndexPath, wantPrefix) {
		t.Fatalf("model-started index path=%q, want Hub path under %q", ready.IndexPath, wantPrefix)
	}
}
...
func TestCtrlSSemanticIndexLeavesLocalRepositoryUntouched(t *testing.T) {
	...
	for _, message := range messages {
		if value, ok := message.(SemanticIndexReadyMsg); ok {
			value := value
			ready = &value
			updated, saveCommand := m.Update(value)
			m = updated.(*Model)
			for _, saveMessage := range runUISemanticCommands(saveCommand) {
				updated, _ = m.Update(saveMessage)
				m = updated.(*Model)
			}
		}
	}
		...
		},
		{
			...
			setup: func(m *Model) {
				...
				catalog := repositorypkg.Catalog{
					{ID: "repo-a", Name: "repo-a", Kind: repositorypkg.IdentityPrefix},
					{ID: "repo-b", Name: "repo-b", Kind: repositorypkg.IdentityPrefix},
				}
