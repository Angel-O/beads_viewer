...
func TestKeyBindingDocsHaveOneEntryPerContextAndKey(t *testing.T) {
	seen := make(map[string]bool)
	for _, doc := range GetKeyBindingDocs() {
		for _, focus := range focusesForBindingDoc(doc) {
			key := fmt.Sprintf("%v\x00%s", focus, doc.Key)
			if seen[key] {
				t.Errorf("duplicate keybinding for focus=%v key=%q", focus, doc.Key)
				continue
			}
			seen[key] = true
		}
...
func TestKeyDispatch_DocumentedKeysChangeState(t *testing.T) {
	...
	t.Run("w without workspace mode explains itself", func(t *testing.T) {
		...
		m = updated.(*Model)
		if !strings.Contains(m.statusMsg, "Hub board") {
			t.Fatalf("expected a Hub-board status message, got %q", m.statusMsg)
		}
	})
