...
func (a *Analyzer) GenerateAdvancedInsightsFromStats(stats *GraphStats, config AdvancedInsightsConfig) *AdvancedInsights {
	...
	insights.ParallelGain = a.generateParallelGain(config.ParallelGainLimit, config.CandidateFilter)
...
func (a *Analyzer) generateTopKSet(k int, predicate CandidatePredicate) *TopKSetResult {
	...
	for _, issue := range a.issueMap {
		if !isClosedLikeStatus(issue.Status) && !issue.IsDeferredAt(a.now) && candidateAllowed(predicate, issue.ID) {
			potentialCandidates++
		}
	}
	...
	for i := 0; i < k; i++ {
		...
		for _, candidate := range actionable {
			candID := candidate.ID
			if !candidateAllowed(predicate, candID) {
				continue
			}
...
func (a *Analyzer) generateCoverageSet(limit int, predicate CandidatePredicate) *CoverageSetResult {
	...
	if predicate != nil {
		capped = false
		for _, e := range uncovered {
			if candidateAllowed(predicate, e.from) || candidateAllowed(predicate, e.to) {
				capped = true
				break
			}
		}
...
func (a *Analyzer) generateParallelGain(limit int, predicate CandidatePredicate) *ParallelGainResult {
	start := time.Now()
	if limit <= 0 {
		limit = 5
	}
	...
	candidateIDs := make([]string, 0, len(actionable))
	for _, iss := range actionable {
		actionableSet[iss.ID] = true
		actionableIDs = append(actionableIDs, iss.ID)
		if candidateAllowed(predicate, iss.ID) {
			candidateIDs = append(candidateIDs, iss.ID)
		}
	}
	...
	if evaluated < len(candidateIDs) {
		status.Capped = true
		status.Reason = fmt.Sprintf("time budget %s exhausted after %d of %d candidates", budget, evaluated, len(candidateIDs))
	}
