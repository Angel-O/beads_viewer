...
func TestAdvancedInsightsCandidateFilterRunsBeforeCaps(t *testing.T) {
	...
	if result.CoverageSet.Status.Capped {
		t.Fatalf("coverage unexpectedly capped by filtered-out edges: %#v", result.CoverageSet.Status)
	}
...
func TestParallelGainCandidateFilterRunsBeforeCap(t *testing.T) {
	issues := []model.Issue{
		{ID: "visible", Status: model.StatusOpen},
		{ID: "hidden-a", Status: model.StatusOpen, Dependencies: []*model.Dependency{{DependsOnID: "visible", Type: model.DepBlocks}}},
		{ID: "hidden-b", Status: model.StatusOpen, Dependencies: []*model.Dependency{{DependsOnID: "visible", Type: model.DepBlocks}}},
	}
	config := DefaultAdvancedInsightsConfig()
	config.ParallelGainLimit = 1
	config.CandidateFilter = func(id string) bool { return id == "visible" }
	result := NewAnalyzer(issues).GenerateAdvancedInsights(config).ParallelGain

	if len(result.Metrics) != 1 || result.Metrics[0].ID != "visible" {
		t.Fatalf("parallel-gain metrics = %#v", result.Metrics)
	}
	if result.Status.Capped || result.Status.Limited != 1 {
		t.Fatalf("parallel-gain status = %#v", result.Status)
	}
}
...
func TestGenerateAdvancedInsightsReportsEffectiveNormalizedConfig(t *testing.T) {
	...
	if insights.Config.TopKSetLimit != want.TopKSetLimit ||
		insights.Config.CoverageSetLimit != want.CoverageSetLimit ||
		insights.Config.KPathsLimit != want.KPathsLimit ||
		insights.Config.PathLengthCap != want.PathLengthCap ||
		insights.Config.CycleBreakLimit != want.CycleBreakLimit ||
		insights.Config.ParallelCutLimit != want.ParallelCutLimit ||
		insights.Config.ParallelGainLimit != want.ParallelGainLimit ||
		insights.Config.CandidateFilter != nil {
